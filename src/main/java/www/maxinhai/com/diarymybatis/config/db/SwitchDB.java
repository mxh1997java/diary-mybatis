...
public class SwitchDB {
     ...
     */
    public String change(String dbName,int dbId)
    {
        if(dbId == 0){
            toDB(dbName);
        }else {
     ...
     * @param dbId
     */
    private void  toYunDB(int dbId){
        //组合私有库数据源对象key
        String dbSourceKey = ljyunDataSourceKey+String.valueOf(dbId);
        //获取当前连接的数据源对象的key
        String currentKey = DynamicDataSourceContextHolder.getDataSourceKey();
        if(dbSourceKey == currentKey) return;

        //创建私有库数据源
        createLjyunDataSource(dbId);

        //切换到当前数据源
        DynamicDataSourceContextHolder.setDataSourceKey(dbSourceKey);
        log.info("＝＝＝＝＝私有库: "+dbId+",切换完毕");
    }
